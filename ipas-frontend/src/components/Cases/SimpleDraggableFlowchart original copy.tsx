...
import EPICNotificationStatus from '../Notifications/EMRNotificationStatus';
...
const SimpleDraggableFlowchart: React.FC<SimpleDraggableFlowchartProps> = ({ caseId }) => {
    ...
    if (caseId === 'PA-2024-001' || caseId === '001') {
      return [
        ...
        },
        {
          ...
          subSteps: ['Letter Creation', 'Letter Generation', 'Provider Notification', 'EMRIntegration'],
          nextSteps: [],
          position: { x: 800, y: 50 }
        }
      ];
  ...
  const startProviderNotificationProcess = () => {
    ...
    // Step 4: EMRIntegration
    setTimeout(() => {
      setShowMessage('Sending authorization data to EMRmedical records...');
      setAnimationStep(36);
    }, 7000);

    setTimeout(() => {
      setShowMessage('✓ Data transmitted to EMRsuccessfully');
      setAnimationStep(37);
      
      // Trigger EMRnotification service immediately
      import('../../services/emrNotificationService').then(({ emrNotificationService }) => {
        // Send to EMRimmediately
      ...
      <CardContent>
        ...
        {showMessage && (
          <Box sx={{ 
            ...
            {/* EMRMedical Record Link */}
            {showMessage.includes('Data transmitted to EMRsuccessfully') && (
              <Box sx={{ mt: 2 }}>
                <Button
                  variant="contained"
                  color="primary"
                  ...
                  onClick={() => {
                    // Open EMRmedical record in new tab
                ...
                <Typography variant="caption" sx={{ display: 'block', mt: 1, color: 'text.secondary' }}>
                  Authorization data has been integrated into the patient's EMRmedical record
                </Typography>
              </Box>
            )}
          </Box>
            ...
            >
              ...
              {step.id === 'provider-notification' && step.status === 'completed' && (
                <Box sx={{ mt: 1, display: 'flex', gap: 1, flexDirection: 'column' }}>
                  ...
                  <EPICNotificationStatus caseId={caseId} />
                </Box>
              )}
            </Paper>
          ))}
